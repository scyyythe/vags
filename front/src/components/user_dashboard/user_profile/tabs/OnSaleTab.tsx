...
const SellTab = () => {
  ...
  const onLikeToggle = useCallback(() => {
    // Optional: hook this into wishlist logic
  }, []);

  const filteredSellArtworks = myArtCards.filter((art) =>
    art.art_status?.toLowerCase() === "onSale" || art.visibility?.toLowerCase() === "public"
  );
  ...
  if (!isLoading && filteredSellArtworks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center col-span-full text-center p-4">
        ...
        <p className="text-sm text-gray-500">You haven’t listed any artworks for sale yet.</p>
      </div>
    );
  }
