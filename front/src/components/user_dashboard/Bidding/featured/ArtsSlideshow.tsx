...
const ArtSlideshow = ({
  ...
  return (
    <div
      className={cn(
        "relative w-full h-full overflow-hidden rounded-2xl bg-black flex items-center gap-12",
        "px-8 py-8", // default padding
        isMobile && "px-2 py-2" // smaller padding on mobile
      )}
    ...
    >
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <div className="flex items-center gap-2">
            <img
              src={currentArtwork.artistAvatar}
            ...
            <span className={cn(
              "font-medium",
              isMobile ? "text-[10px]" : "text-xs"
            )}>{currentArtwork.artist}</span>
          </div>
        </div>

        <div className={cn(
          "flex items-center gap-16 mt-4"
        )}>
            <button className={cn(
